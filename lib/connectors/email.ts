...
export class Web3AuthEmailConnector extends Connector<SafeEventEmitterProvider, Options> {
  ready = !IS_SERVER;
    ...
    try {
      ...
      if (!this.web3AuthInstance.connected) {
        if (isIWeb3AuthModal(this.web3AuthInstance)) {
          ...
          await this.web3AuthInstance.connectTo(WALLET_ADAPTERS.OPENLOGIN, loginParameters);
        } else {
