...
import { DelegatedAmountsByDelegators, DelegatedAmountsRequest, Operator, OperatorContactDetail, Paymaster, SignData, TokenBalances, ValidatorResponse } from "./types";
...
export const fetchAddressTokenBalances = (address: Address): Promise<TokenBalances> =>
    axios.get(`https://explorer.fuse.io/api/v2/addresses/${address}/token-balances`).then(response => response.data)